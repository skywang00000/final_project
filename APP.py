...
def binary_search_visual(arr_text, target):
    # Binary search method
    try:
        arr = [int(x.strip()) for x in arr_text.split(",") if x.strip() != ""]  # Parsing the input array
        target = int(target)
    except Exception:
...
def random_array(size, min_val, max_val):
    ...
    arr = sorted([random.randint(min_v, max_v) for _ in range(size)])  # Generate sorted array
    return ",".join(str(x) for x in arr)

def random_array_simple():
    # Simple random array generator
    return random_array(8, -10, 10)
